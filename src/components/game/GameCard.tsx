...
export default function GameCard({ game }: Props) {
  return (
    <Link  className="flex flex-col bg-surface rounded-xl overflow-hidden transition-all duration-200 hover:bg-surface-hover cursor-pointer" to={`/games/${game.id}`}>
          ...
          <footer>
            <ul className="flex flex-row gap-2 flex-wrap">
              {game.genres.map((genre) => (
                <li
                  key={genre.id}
                  className="font-medium text-lg text-tx-muted"
                >
                  {genre.name}
                </li>
              ))}
